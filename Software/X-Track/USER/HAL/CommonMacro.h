...
/**
  * @brief  让一个变量增加或者减少一个值，在大于等于最大值后从0开始，低于0后从最大值开始
  * @param  src: 被控变量
  * @param  step: 增加或者减少的值
  * @param  max: 最大值
  * @retval 无
...
do {                                           \
    if ((step) >= 0) {                         \
        ...
        (src) = ((src) + (max) + (step)) % (max); \
    }                                          \
} while(0)

/**
  * @brief  让一个变量增加或者减少一个值，在大于最大值后从最小值开始，小于最小值后从最大值开始
  * @param  src: 被控变量
  * @param  plus: 增加的值
  * @param  min: 最小值
  * @param  max: 最大值
...
/**
  * @brief  获取信号量，当 sem 为 true 时执行一次 func
  * @param  sem: 信号量 (bool 类型)
  * @param  func: 被调用函数 (也可以是赋值等其他语句)
  * @retval 无
  */
